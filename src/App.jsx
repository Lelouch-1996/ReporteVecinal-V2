...
function seedIfEmpty() {
  if (!localStorage.getItem(CATALOGS_KEY)) {
    ApiService.saveCatalogs(DEFAULT_CATALOGS);
  }

  if (!localStorage.getItem(REPORTS_KEY)) {
    ApiService.saveReports(makeSeedReports());
  }

  if (!localStorage.getItem(NOTIFICATIONS_KEY)) {
    ApiService.saveNotifications([
      {
        id: crypto.randomUUID(),
...
export default function App() {
  ...
  function syncCatalogs(next) {
    setCatalogs(next);
    ApiService.saveCatalogs(next);
  }

  function syncNotifications(next) {
    setNotifications(next);
    ApiService.saveNotifications(next);
  }
